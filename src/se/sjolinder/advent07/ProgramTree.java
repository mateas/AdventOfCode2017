...
public class ProgramTree {
    ...
    public static String part1(String list) {
        Node root = parse(list);
        return root.name;
    }

    public static int part2(String list) {
        Node n = parse(list);
        ...
        while (!queue.isEmpty()) {
            ...
            HashMap<Integer, ArrayList<Node>> weights = weights(node);
            if (weights.keySet().size() == 1) {
                n = weights.entrySet().iterator().next().getValue().get(0).parent;
                break;
            }


        }
        HashMap<Integer, ArrayList<Node>> weights = weights(n.parent);
        Node errorNode = n.parent;
        Node rightNode = n.parent;

        for (Map.Entry<Integer, ArrayList<Node>> e : weights.entrySet()) {

            if (e.getValue().size() == 1) {
                errorNode = e.getValue().get(0);
            } else {
                rightNode = e.getValue().get(0);

            }
        }
        return rightNode.sum() - errorNode.sumChilds();
    }

    private static HashMap<Integer, ArrayList<Node>> weights(Node node) {
        HashMap<Integer, ArrayList<Node>> weights = new HashMap<Integer, ArrayList<Node>>();
        for (Node child : node.childs) {

            int sumTot = child.sum();
            if (!weights.containsKey(sumTot)) {
                weights.put(sumTot, new ArrayList<>());
            }
            weights.get(sumTot).add(child);
        }
        return weights;
    }

    public static Node parse(String list) {
        String[] rows = list.split(System.getProperty("line.separator"));
        ArrayList<Node> allNodes = new ArrayList<>();
        Pattern pattern = Pattern.compile("(\\w+)\\s\\((\\d+)\\)");
        for (int i = 0; i < rows.length; i++) {
            Node node = new Node();
            allNodes.add(node);
            String nameWeight = rows[i];
            if (rows[i].contains(" -> ")) {
                String[] split = rows[i].split(" -> ");
                nameWeight = split[0];
                node.childNames = split[1].split(", ");
            }
            Matcher matcher = pattern.matcher(nameWeight);
            matcher.find();
            node.name = matcher.group(1);
            node.weight = Integer.parseInt(matcher.group(2));
        }

        return sort(allNodes);
    }

    private static Node sort(ArrayList<Node> nodes) {

        for (Node node : nodes) {
            if (node.childNames.length > 0) {
                for (int i = 0; i < node.childNames.length; i++) {
                    for (Node child : nodes) {
                        if (child.name.equals(node.childNames[i])) {
                            child.parent = node;
