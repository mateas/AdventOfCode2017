...
public class Checksum {
    ...
    public static void main(String[] args) {
        ...
        String spreadsheet = "157\t564\t120\t495\t194\t520\t510\t618\t244\t443\t471\t473\t612\t149\t506\t138\n" +
    ...
    public static int part1(String spreadsheet) {
        int[][] matrix = parseSpreadsheetString(spreadsheet);
        return part1(matrix);
    }

    public static int part1(int[][] matrix) {
        int sum = 0;
    ...
    public static int part2(String spreadsheet) {
        int[][] matrix = parseSpreadsheetString(spreadsheet);
        return part2(matrix);
    }

    public static int part2(int[][] matrix) {
        int sum = 0;
        for (int i = 0; i < matrix.length; i++) {
            int[] row = matrix[i];
            for (int j = 0; j < row.length; j++) {
                for (int k = 0; k < row.length; k++) {
                    if (j != k && row[j] % row[k] == 0) {
                        sum = sum + row[j] / row[k];
                    }
                }
            }
    ...
    private static int[][] parseSpreadsheetString(String spreadsheet) {
        String[] rows = spreadsheet.split(System.getProperty("line.separator"));
        int[][] matrix = new int[rows.length][];
        for (int i = 0; i < rows.length; i++) {
            String[] cells = rows[i].split("\\s+");
            int[] cellsDigits = new int[cells.length];
            matrix[i] = cellsDigits;
            for (int j = 0; j < cells.length; j++) {
                cellsDigits[j] = Integer.parseInt(cells[j]);
            }
        }
