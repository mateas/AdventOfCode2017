...
public class PassPhrase {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Advent04: part1 1: " + part1List(input));
        System.out.println("Advent04: part1 2: " + part2List(input));
    }

    public static int part1List(String list) {
        String[] phrases = parseList(list);
        int passes = 0;
    ...
    public static boolean part1(String phrase) {

        String[] words = phrase.split("\\s+");
        for (int i = 0; i < words.length; i++) {
            for (int j = i + 1; j < i + words.length - 1; j++) {
                if (words[i].equals(words[j % words.length]))
                    return false;
            }
        }
        return true;
    ...
    public static int part2List(String list) {
        String[] phrases = parseList(list);
        int passes = 0;
        for (String phrase : phrases) {
            if (part2(phrase))
    ...
    public static boolean part2(String phrase) {

        String[] words = phrase.split("\\s+");
        for (int i = 0; i < words.length; i++) {
            for (int j = i + 1; j < i + words.length - 1; j++) {
                if (part2Equals(words[i], words[j % words.length]))
                    return false;
            }
        }
        return true;
    ...
    private static boolean part2Equals(String wordA, String wordB) {
        if (wordA.length() != wordB.length())
            return false;
        ArrayList<Character> letters =  new ArrayList<Character>();// wordB.toCharArray());
        for(char c : wordB.toCharArray()) {
            letters.add(c);
        }

        for (int i = 0; i < wordA.length(); i++) {
            int index = letters.indexOf(wordA.charAt(i));
            if (index >= 0){
