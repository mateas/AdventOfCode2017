...
public class Instructions {
    public static void main(String[] args) {
        ...
        String input = "d dec 461 if oiy <= 1\n" +
        ...
        System.out.println("Advent08: part1 1: " + part1(input));
        System.out.println("Advent08: part1 2: " + part2(input));
    }
    ...
    public static int part1(String input) {
        ...
        for (int i = 0; i < rows.length; i++) {
            Pattern pattern = Pattern.compile("(\\w+)\\s+(\\w+)\\s+(-?\\d+)\\s+if\\s+(\\w+)\\s+(\\S+)\\s+(-?\\d+)");
            Matcher matcher = pattern.matcher(rows[i]);
            matcher.find();
            String variableName = matcher.group(1);
            if (!register.containsKey(variableName))
                register.put(variableName, 0);
            String variableNameCond = matcher.group(4);
            if (!register.containsKey(variableNameCond))
                register.put(variableNameCond, 0);
            ...
            try {
                engine.eval(variableNameCond + " = " + register.get(variableNameCond));
                Boolean result = (Boolean) engine.eval(variableNameCond + " " + matcher.group(5) + " " + matcher.group(6));
                if (result) {
                    int value = Integer.parseInt(matcher.group(3));
                    if (matcher.group(2).equals("inc"))
                        register.put(variableName, register.get(variableName) + value);
                    else
                        register.put(variableName, register.get(variableName) - value);
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (result) {
                    ...
                    if (highestValue < register.get(variableName) )
                        highestValue = register.get(variableName) ;
                }
            } catch (ScriptException e) {
                e.printStackTrace();
            }
